(() => {
  ...
  function bindEvents() {
    ...
    els.zoomVietnamButton.addEventListener("click", () => {
      ...
      setStatus("Đang xem toàn Việt Nam.");
    });
    els.clearMarkersButton.addEventListener("click", () => {
      clearRouteGeometry();
      clearTemporaryMarkers();
      setStatus("Đã ẩn ghim tạm.");
    });
  ...
  function fillRouteOriginWithLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        applyLiveNavigationPosition(position, { keepVisible: false });
        startLiveNavigation();
        if (routeState.active) {
          previewRoute();
        } else {
          setStatus("Đã bật điểm đi theo vị trí hiện tại.");
        }
      },
      (error) => {
  ...
  async function shareCurrentView() {
    ...
    try {
      ...
      window.prompt("Liên kết bản đồ", link.href);
    }
  }
